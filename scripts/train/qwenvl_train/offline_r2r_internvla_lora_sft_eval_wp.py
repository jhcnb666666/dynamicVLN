...
def load_model_with_lora(args):
    ...
    # InternVLA-N1 checkpoint has model_type 'internvla_n1' which transformers
    # does not recognise. We force-load it as Qwen2.5-VL by reading the raw
    # config dict and building a Qwen2_5_VLConfig from it.
...
def parse_args():
    ...
    parser.add_argument("--waypoint_threshold", type=float, default=None,
                        help="If set, compute waypoint filtering stats on val_root pose files with this threshold.")
...
def main():
    ...
    if args.waypoint_threshold is not None:
...
def compute_waypoint_filter_stats(val_root: str, max_eval_samples: int, threshold: float, seed: int) -> Dict[str, float]:
    """Load poses from val_root episodes and compute waypoint filtering statistics."""
